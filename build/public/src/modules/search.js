...
// const { sendNameChangeNotification } = require("../../../src/user/notifications");

define('search', [
	'translator', 'storage', 'hooks', 'alerts', 'bootstrap', 'utils', 
	...
	Search.displayHistory = function(dropdownEl, inputEl) {
		const history = Search.getHistory();

		if(!history || history.length === 0){
			dropdownEl.addClass('hidden').empty();
			return;
		}
		...
		historyHTML += '</div>';
		historyHTML += '<div class="search-history-clear">';
		historyHTML += '<button class="clear-history-btn">Clear search history</button>';
		historyHTML += '</div>';
		historyHTML += '</div>';

		dropdownEl.removeClass('hidden').html(historyHTML);

		dropdownEl.find('.search-history-item').on('click', function(){
		...
		dropdownEl.find('.clear-history-btn').on('click', function(e){
			e.preventDefault();
			e.stopPropagation();
			Search.clearHistory();
	...
	Search.init = function (searchOptions) {
		...
		$(document).on('focus', '[component="search/fields"] input[name="query"]',function(){
			var input =$(this);
			var dropdown = input.closest('[component="search/fields"]').find('#quick-search-container');
			var query = input.val();

			if(!query || query.length === 0){
				Search.displayHistory(dropdown,input);
	...
	Search.showHistoryDropdown = function(dropdownEl,inputEl,searchOptions){
		Search.displayHistory(dropdownEl,inputEl);
	};
	...
	Search.enableQuickSearch = function (options) {
		...
		inputEl.on('focus', function () {
			console.log('focus on fired event');
